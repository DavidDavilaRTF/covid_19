...
class prediction_covid:
    ...
    def mesure(self):
        for i in range(self.cv):
            ...
            self.mes['lm_1'].iloc[0] += self.mes['lm_2'].iloc[0] + 1
...
class prod:
    ...
    def prod_pred(self):
        ...
        x_pred = numpy.array(self.prod_file)

        sel = x_train.astype(str) == 'inf'
        x_train[sel] = 0
        sel = sel.astype(int)
        x_train = numpy.c_[x_train,sel]

        sel = x_train.astype(str) == 'nan'
        x_train[sel] = 0
        sel = sel.astype(int)
        x_train = numpy.c_[x_train,sel]

        sel = x_pred.astype(str) == 'inf'
        x_pred[sel] = 0
        sel = sel.astype(int)
        x_pred = numpy.c_[x_pred,sel]

        sel = x_pred.astype(str) == 'nan'
        x_pred[sel] = 0
        sel = sel.astype(int)
        x_pred = numpy.c_[x_pred,sel]

        x_train = pandas.DataFrame(x_train)
        x_pred = pandas.DataFrame(x_pred)
